...
if __name__ == '__main__':
    print('hello')
    img = cv2.imread("D:/RGBW_denoising/data/move_data/png/rgbw_norm_"+ str(0)+".png",0)[:,:,np.newaxis].astype('float64') *4
    img2 = cv2.imread("D:/RGBW_denoising/data/move_data/png/rgbw_norm_"+ str(1)+".png",0)[:,:,np.newaxis].astype('float64') *4
    frame = np.concatenate((img,img2),axis=2)
    alignment = align(frame)
    output = merge_temporal(frame,alignment)
    ...
    cv2.imwrite("D:/RGBW_denoising/data/move_data/png/test_norm_merge.png",output / 4)
